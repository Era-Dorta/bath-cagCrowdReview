...
For instance, in the \textit{films} industry where it is not always possible or economically viable to have a real crowd, in the \textit{video games} industry as a crowd might be required in a virtual world and lastly, in \textit{disaster prevention and management} where they are used to aid in the decision making process as crowds provide new sources of data.\\ %Disaster prevention rephrase entirely, say how the data is provided, by behavour simulation
...
A clear line can be drawn between real-time simulation (games) and non real-time simulation (films). % ,e.g films, or (e.g. films)
...
Regarding time, we have short-term simulations, medium-term simulations and long-term simulations. %Totally redundant, is only saying we can move
While in terms of size, a distinction can be made among small, medium and big size crowds.\\ % in those areas
...
Hence, it will typically be considered only after a certain density threshold is reached.\\ %Not clear, rephrase, only used in dense crowds
...
Firstly, \textbf{dynamic mesh decimation} involves adopting simpler mesh representations. %If going to enumarate, then use the begin enumarate.
...
Before the crowd can be simulated or rendered it has to be generated. %Too obvious
...
For example, crowd size, goals, etc; other visual parameters could also be included, such as color or meshes  for the agents.\\ %Add a couple more examples

%Always start with the first most obvious inneficient approach, paper
...
Therefore, an intuitive approach is provided, as it is similar to what users are used to. %Say explicitly brush metaphor is from another areas
...
%Do a time list with the most important things on each area, the way to do it is, this person did this based on this and this other 3 or 4 group citations
%used/improved the method

%Also can say, following this survey this and this happend, and more recently this and this

%Make clear the conecction between the different papers
